...
`.claude/hooks/i18n-parity.sh` rejects a message file edit that leaves the two
languages out of step, instead of that surfacing three screens later.

`.claude/hooks/session-brief.sh` prints the repository's shape at session start.
Otherwise every fresh session spends thousands of tokens rediscovering the FSD
layout, and occasionally guesses which layer something belongs in.
...
- **Point at the file.** Naming `src/entities/invoice/model/status-tone.ts` is
  cheaper than describing what it does and letting it be searched for.
